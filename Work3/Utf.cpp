...
{
    namespace {
        constexpr auto not_detect_character = '\0';
    }
    ...
    namespace detail {
        ...
        size_t UtfEncoding<char>::decode(const types::byte_t *src, size_t n, char32_t &dst) noexcept {
            ...
            switch (code[0]) {
                case 0xe0:
                    if (code[1] < 0xa0 || code[1] > 0xbf) return 1;
                    break;
                case 0xed:
                    if (code[1] < 0x80 || code[1] > 0x9f) return 1;
                    break;
                case 0xf0:
                    if (code[1] < 0x90 || code[1] > 0xbf) return 1;
                    break;
                case 0xf4:
                    if (code[1] < 0x80 || code[1] > 0x8f) return 1;
                    break;
                default:
                    if (code[1] < 0x80 || code[1] > 0xbf) return 1;
                    break;
            }
            if (code[0] <= 0xdf) {
        ...
        size_t UtfEncoding<char16_t>::decode(const char16_t *src, size_t n,
           ...
           if (char_is_surrogate(src[0]) || char_is_bmp(src[0])) {
            ...
            else if (n >= 2 && char_is_high_surrogate(src[0])
                     && char_is_low_surrogate(src[1])) {
        ...
        size_t UtfEncoding<char16_t>::decode_prev(const char16_t *src, size_t pos, char32_t &dst) noexcept {
            ...
            else if (pos >= 2 && char_is_high_surrogate(src[pos - 2])
                     && char_is_low_surrogate(src[pos - 1])) {
